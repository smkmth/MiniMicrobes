    ...
    public void InitCells()
    {
        ...
        for (int row = 0; row < rows; row++)
        {
            for (int col = 0; col < cols; col++)
            {
                ...
                //add a collider so we can click on the grid
                ...
                //give the elements on the grid names and a common tag
                cO.name = col.ToString() + " , " + row.ToString();
                cO.tag = "GridSquare";
                //set the element to a layer that it can only interact with itself
                cO.layer = 9;
                GridCell gridcell = cO.AddComponent<GridCell>();
                gridcell.x = row;
                gridcell.y = col;
